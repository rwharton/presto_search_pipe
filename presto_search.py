...
import sys
import time
import search_params as params
import sifting
import re
import shlex
import psr_utils
...
def check_dependencies(work_dir, fits_dir, fitsbase):
    ...
    """
    # Get data type (fits or fil)
    if params.dat_type == 'psrfits':
        suffix = 'fits'
    else:
        suffix = 'fil'
    ...
    fl = glob(fits_dir + '/%s*.%s' %(fitsbase, suffix))
    if len(fl):
        print("  Found %d file(s) in %s:\n" %(len(fl), fits_dir))
        for i in fl:
            print("    %s\n" %(i.split('/')[-1]))
    else:
        print("  No %s*.%s files found in %s !\n   Exiting...\n" %(\
                                         fitsbase, suffix, fits_dir))

        if (params.do_rfifind or params.do_prepsub):
            print("  ... Need FITS or FIL files for selected mode(s) \n")
            print("  ... Exiting .... \n")
            sys.exit(0)
        else:
...
def try_cmd(cmd, stdout=None, stderr=None):
    ...
    print("\n\n %s \n\n" %cmd)
    try:
        retval = sp.run(cmd, shell=True, stdout=stdout, stderr=stderr, check=True)
    except sp.CalledProcessError:
        print("The command:\n %s \ndid not work, quitting..." %cmd)
        sys.exit(0)

def run_rfifind(fitslist, fitsname, work_dir):
    print("Running rfifind on the filterbank files")
    ...
    cmd = 'rfifind -o %s -time %d -timesig %f -freqsig %f '\
...
def run_prepsubband(basename, maskname, fitslist, dmlow=params.dmlow, \
    ...
    if params.use_mask:
        cmd = 'prepsubband -o %s -nsub %d -numout %d -lodm %.6f -dmstep %.6f '\
...
def accelsift(filenm):
    ...
    if len(re.findall("_[0-9][0-9][0-9]M_" , inffiles[0])):
        ...
        dmstrs = [x.split("DM")[-1].split(".inf")[0] for x in inffiles]
    dms = [ float(dmstr) for dmstr in dmstrs ]
    dms.sort()
    dmstrs = ["%.2f"%x for x in dms]
    ...
    if len(all_cands):
        ...
        all_cands.sort(key=attrgetter('sigma'), reverse=True)
...
def run_realfft(workdir, basename):
    ...
    for datfile in datfiles:
        ...
        if os.path.exists(current_nm):
            ...
            arg_tup = (datfile, "fft.out", "fft.err")
...
def run_one_accel(datfile, zmax, nharm, flo, fhi, sigma_min, zap_str, suffix):
    ...
    accel_out = open(fname+'_accel.out', 'w')
    accel_err = open(fname+'_accel.err', 'w')
...
def run_accelsearch(work_dir, basename):
    ...
    accel_out = 'accelsearch.out'
    accel_err = 'accelsearch.err'
...
def search_beam(fitsname, fits_dir, work_dir):
    ...
    if params.do_rfifind or params.do_prepsub:
        fitslist = glob('%s/%s*.%s' %(fits_dir, fitsname, suffix))
        fitslist.sort()
        print(fitslist)
...
def parse_input():
    ...
    """
    prog_desc = "PRESTO Search Pipeline"
    parser = ArgumentParser(description=prog_desc)
    parser.add_argument('indir',
                        help='Directory containing *.fil or *fits files')
    parser.add_argument('outdir',
                        help='Output directory for results')
    parser.add_argument('basename',
                        help='Basename for input data files')
    args = parser.parse_args()
    indir = args.indir  
    outdir = args.outdir
    basename = args.basename
    return indir, outdir, basename
...
if __name__ == "__main__":
    ...
    in_fits_dir, in_results_dir, basename = parse_input()

    # Convert directories to absolute paths
    fits_dir = os.path.abspath(in_fits_dir)
    results_dir = os.path.abspath(in_results_dir)

    print("fits_dir: %s" %fits_dir)
    print("results_dir: %s" %results_dir)
    
    print(glob("%s/*" %fits_dir))
    print(glob("%s/*" %results_dir))

    # Actually do the search
    search_beam(basename, fits_dir, results_dir)
