...
def run_prepfold_topcands(filenm, fits_dir, search_dir, zmax, wmax, outfile, 
                          errfile, searchtype='none', start=None, stop=None):
    """
    ...
    for line in f:
        ...
        if len(namesplit) != 2:
            ...
            outname = "cand%03d_"%ii + "_" + namecand.split("_ACCEL_")[0] + "z%d_" %zmax + "w%d" %wmax
            psname  = "%s_JERK_Cand%s.pfd.ps" %(outname, cnum)
...
############
#sys.exit()
top_dir = "/hercules/results/rwharton/phased_gc/day1/search_jerk/sift_test"
cands_dir = "%s/cands_presto" %(top_dir)
best_dir = "%s/top_plots" %(top_dir)
cand_file = "%s/sgra_day1.ACCEL_300_JERK_900.sifted.cands" %(cands_dir)
good_file = "%s/good_cands.txt" %(best_dir)
...
zmax = 300
wmax = 900
run_prepfold_topcands(good_file, fits_dir, search_dir, zmax, wmax, None, None, 
                      searchtype='none', start=start, stop=stop)
